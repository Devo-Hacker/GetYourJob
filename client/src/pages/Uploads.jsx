...
function ProjectActions({ project, onEdit, onDelete }) {
  return (
    <div className="flex items-center gap-3 text-slate-400">
      ...
      <button onClick={() => onEdit(project)} aria-label="Edit project" className="hover:text-indigo-500">
        <Pencil size={15} />
      </button>
      <button onClick={() => onDelete(project.id)} aria-label="Delete project" className="hover:text-rose-500">
        <Trash2 size={15} />
...
function ProjectListRow({ project, onEdit, onDelete }) {
  return (
    <ClayCard className="p-4 flex flex-col sm:flex-row sm:items-center gap-3">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2.5 flex-wrap">
...
function AddProjectModal({ open, editProject, onClose, onSave }) {
  const [form, setForm] = useState({
    name: "",
    description: "",
    techStack: "",
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={editProject ? "Edit Project" : "Add New Project"}
        className="relative w-full max-w-md bg-white rounded-[28px] shadow-2xl max-h-[90vh] overflow-y-auto p-6"
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-slate-800">{editProject ? "Edit Project" : "Add New Project"}</h2>
          <button onClick={onClose} aria-label="Close" className="text-slate-400 hover:text-slate-600">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1 pl-1">
              Add or remove any tech here anytime — separate each with a comma.
            </p>
          </div>
          <select
            value={form.status}
          ...
          <div>
            <label className="text-[11.5px] font-medium text-slate-500">
              {editProject ? "Replace attached file (optional)" : "Attach a file (screenshot, zip, doc - optional)"}
            </label>
            <input
              type="file"
              onChange={(e) => setFile(e.target.files?.[0] || null)}
          ...
          <button
            ...
            className="w-full py-2.5 rounded-2xl bg-indigo-600 text-white text-[13px] font-semibold shadow-[0_10px_20px_-8px_rgba(79,70,229,0.5)] disabled:opacity-60"
          >
            {submitting ? (editProject ? "Saving..." : "Adding...") : editProject ? "Save Changes" : "Add Project"}
          </button>
        </form>
      </div>
    </div>,
  ...
  return (
    <>
      ...
      <ProjectsToolbar
        ...
        onAddClick={openAdd}
      />
      <StatsRow stats={projectsData.stats} onAddClick={openAdd} />

      {view === "grid" ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
          {filteredProjects.map((p) => (
            <ProjectCard key={p.id} project={p} onEdit={openEdit} onDelete={handleDeleteProject} />
          ))}
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {filteredProjects.map((p) => (
            <ProjectListRow key={p.id} project={p} onEdit={openEdit} onDelete={handleDeleteProject} />
          ))}
        </div>
      )}
